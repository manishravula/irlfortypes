...
parser.add_argument("--mcts_setting",type=str,help='MCTS setting type. heuristic\n absolute \n')
        ...
        while (j<n_max_iters_in_experiment) and not main_arena.isterminal:
            ...
            if args.mcts_setting == 'heuristic':
                if config.N_MAXITERS_IN_EXPERIMENTS-j>config.MAX_HEURISTIC_ROLLOUT_DEPTH:
                    rollout_depth = config.MAX_HEURISTIC_ROLLOUT_DEPTH
                else:
                    rollout_depth = config.N_MAXITERS_IN_EXPERIMENTS - j
            else:
                rollout_depth = config.MAX_ROLLOUT_DEPTH


            if args.type == 'nocp_wrongestim':
                #randomizing estimates
